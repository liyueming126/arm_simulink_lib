function batch_build_test_fixed(varargin)
% batch_build_test_fixed  编译/测试 fixed 目录下的定点 MEX 函数
%
%    batch_build_test_fixed              编译 + 测试 (默认)
%    batch_build_test_fixed all          编译 + 测试
%    batch_build_test_fixed compile      仅编译所有
%    batch_build_test_fixed test         仅测试所有 (需已有 .mexw64)
%    batch_build_test_fixed failed       仅重新编译和测试失败项 (读取日志)
%    batch_build_test_fixed folderName   编译+测试指定文件夹 (如 transform)
%    batch_build_test_fixed compile folderName  仅编译指定文件夹
%    batch_build_test_fixed test   folderName  仅测试指定文件夹
%
% 结果日志: fixed/batch_test_results.txt
% 编译日志: fixed/batch_build_log.txt
%
...
function build_status = compile_all(c_list, N, script_dir)
    ...
    fprintf(fid, '=== MEX 编译日志 (fixed) ===  %s\n\n', datestr(now));

    fprintf('============================================================\n');
    fprintf('  编译: %d 个 *_mex.c (fixed)\n', N);
    fprintf('============================================================\n');
