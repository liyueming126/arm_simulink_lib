function arm_abs_q31()
clc;
fprintf('=== Testing arm_abs_q31 ===\n\n');
errors = 0;
...
tic; y = arm_abs_q31_mex(x); t = toc;
fprintf('Test 2 - Random %d-element vector: %.3f ms\n', n, t*1000);
ref = x;
neg_mask = (x < 0);
ref(neg_mask & x ~= intmin('int32')) = -x(neg_mask & x ~= intmin('int32'));
ref(x == intmin('int32')) = intmax('int32');
if isequal(y, ref)
    fprintf('  PASS\n\n');
else
