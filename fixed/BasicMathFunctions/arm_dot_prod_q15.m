function arm_dot_prod_q15()
clc;
fprintf('=== Testing arm_dot_prod_q15 ===\n\n');
errors = 0;

% Test 1: basic vectors
x = int16([1, 2, 3, 4, 5]);
y = int16([10, 20, 30, 40, 50]);
z = arm_dot_prod_q15_mex(x, y);
fprintf('Test 1 - Basic vectors:\n  x: ['); fprintf('%d ', x); fprintf(']\n  y: ['); fprintf('%d ', y); fprintf(']\n  result: %.0f\n', z);
ref = sum(int32(x) .* int32(y));
...
y = int16([32767, -32768, -32768, 32767]);
z = arm_dot_prod_q15_mex(x, y);
ref = int64(int32(x(1))*int32(y(1)) + int32(x(2))*int32(y(2)) + int32(x(3))*int32(y(3)) + int32(x(4))*int32(y(4)));
...
tic; z = arm_dot_prod_q15_mex(x, y); t = toc;
fprintf('Test 4 - Random %d-element: %.3f ms\n', n, t*1000);
ref = int64(0);
for i = 1:n, ref = ref + int64(int32(x(i)) * int32(y(i))); end
if double(z) == double(ref)
    fprintf('  PASS\n\n');
else
    fprintf('  FAIL - expected: %ld\n\n', ref);
...
ref = int64(0);
for i = 1:4, ref = ref + int64(int32(x(i)) * int32(y(i))); end
if double(z) == double(ref)
    fprintf('  PASS\n\n');
else
    fprintf('  FAIL\n\n');
