...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (nrhs != 1)
        mexErrMsgIdAndTxt("arm_negate_q7:invalidInput", "1 input required.");
    if (nlhs > 1)
        mexErrMsgIdAndTxt("arm_negate_q7:invalidOutput", "At most 1 output allowed.");
    if (!mxIsInt8(prhs[0]) || mxIsComplex(prhs[0]) || mxIsSparse(prhs[0]))
        mexErrMsgIdAndTxt("arm_negate_q7:invalidInputType", "Input must be real int8.");
    ...
    arm_negate_q7(pSrc, pDst, (uint32_t)blockSize);
}