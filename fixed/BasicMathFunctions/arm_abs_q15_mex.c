...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (nrhs != 1)
        mexErrMsgIdAndTxt("arm_abs_q15:invalidInput", "1 input required.");
    if (nlhs > 1)
        mexErrMsgIdAndTxt("arm_abs_q15:invalidOutput", "At most 1 output allowed.");
    if (!mxIsInt16(prhs[0]) || mxIsComplex(prhs[0]) || mxIsSparse(prhs[0]))
        mexErrMsgIdAndTxt("arm_abs_q15:invalidInputType", "Input must be real int16.");
    ...
    arm_abs_q15(pSrc, pDst, (uint32_t)blockSize);
}