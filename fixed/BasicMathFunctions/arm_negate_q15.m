function arm_negate_q15()
clc;
fprintf('=== Testing arm_negate_q15 ===\n\n');
errors = 0;

% Test 1: basic vector
x = int16([0, 100, -100, 32767, -32767, -32768, 1, -1]);
y = arm_negate_q15_mex(x);
fprintf('Test 1 - Basic vector:\n  Input:  ['); fprintf('%d ', x); fprintf(']\n  Output: ['); fprintf('%d ', y); fprintf(']\n');
ref = int16([0, -100, 100, -32767, 32767, 32767, -1, 1]);
if isequal(y, ref)
    fprintf('  PASS\n\n');
else
...
tic; y = arm_negate_q15_mex(x); t = toc;
fprintf('Test 2 - Random %d-element vector: %.3f ms\n', n, t*1000);
ref = x;
ref(ref == intmin('int16')) = intmax('int16');
ref(ref ~= intmax('int16')) = -ref(ref ~= intmax('int16'));
if isequal(y, ref)
    fprintf('  PASS\n\n');
else
...
ref = int16([-1 2 -3; 4 -5 6]);
if isequal(y, ref), fprintf('  PASS\n\n'); else fprintf('  FAIL\n\n'); errors = errors + 1; end

% Test 4: saturation edge case
x = int16([intmin('int16'), intmax('int16')]);
y = arm_negate_q15_mex(x);
fprintf('Test 4 - Saturation (0x8000 -> 0x7FFF): Input: %d, Output: %d\n', x(1), y(1));
if y(1) == intmax('int16')
    fprintf('  PASS\n\n');
else
    fprintf('  FAIL\n\n');
