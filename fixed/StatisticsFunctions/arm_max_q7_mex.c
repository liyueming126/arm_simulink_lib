...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (nrhs != 1)
        mexErrMsgIdAndTxt("arm_max_q7:invalidInput", "1 input required.");
    if (nlhs > 2)
        mexErrMsgIdAndTxt("arm_max_q7:invalidOutput", "At most 2 outputs allowed.");
    if (!mxIsInt8(prhs[0]) || mxIsComplex(prhs[0]) || mxIsSparse(prhs[0]))
        mexErrMsgIdAndTxt("arm_max_q7:invalidInputType", "Input must be real int8.");

    blockSize = mxGetNumberOfElements(prhs[0]);
    pSrc = (int8_t *)mxGetData(prhs[0]);

    arm_max_q7(pSrc, (uint32_t)blockSize, &resultVal, &resultIdx);
