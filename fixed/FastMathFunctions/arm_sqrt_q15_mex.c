...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (nrhs != 1) {
        mexErrMsgIdAndTxt("arm_sqrt_q15:input", "One input required.");
    }
    if (!mxIsInt16(prhs[0])) {
        mexErrMsgIdAndTxt("arm_sqrt_q15:input", "Input must be int16.");
    }

    N = mxGetNumberOfElements(prhs[0]);
    in = (int16_t *)mxGetData(prhs[0]);

    plhs[0] = mxCreateNumericMatrix(1, (mwSize)N, mxINT16_CLASS, mxREAL);
    out = (int16_t *)mxGetData(plhs[0]);

    for (i = 0; i < N; i++) {
        int16_t val;
        arm_sqrt_q15_scalar(in[i], &val);
        out[i] = val;
    }
}