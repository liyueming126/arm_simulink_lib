...
static const int16_t sinTable_q15[FAST_MATH_TABLE_SIZE + 1] = {
...
static int16_t arm_sin_q15(int16_t x)
{
    int16_t sinVal;
    int32_t index;
    int16_t a, b;
    ...
    sinVal = (int32_t)(0x8000L - fract) * a >> 16;
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (nrhs != 1) {
        mexErrMsgIdAndTxt("arm_sin_q15:input", "One input required.");
    }
    if (!mxIsInt16(prhs[0]) || mxGetNumberOfElements(prhs[0]) != 1) {
        mexErrMsgIdAndTxt("arm_sin_q15:input", "Input must be a scalar int16.");
    }

    x = (int16_t)(*(int16_t *)mxGetData(prhs[0]));
    y = arm_sin_q15(x);

    plhs[0] = mxCreateNumericMatrix(1, 1, mxINT16_CLASS, mxREAL);
    *(int16_t *)mxGetData(plhs[0]) = y;
}