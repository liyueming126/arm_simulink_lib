...
{
    if (nrhs != 1) mexErrMsgIdAndTxt("cmplx_mag_q31:input", "One input required.");
    if (nlhs > 1) mexErrMsgIdAndTxt("cmplx_mag_q31:output", "One output required.");

    if (!mxIsInt32(prhs[0]) || mxGetNumberOfDimensions(prhs[0]) != 2 || mxGetM(prhs[0]) != 1)
        mexErrMsgIdAndTxt("cmplx_mag_q31:input", "Input must be 1xN int32 vector.");

    mwSize numElements = mxGetN(prhs[0]);
    if (numElements % 2 != 0)
        mexErrMsgIdAndTxt("cmplx_mag_q31:input", "Input length must be even.");
    ...
    arm_cmplx_mag_q31(src, dst, numSamples);
}