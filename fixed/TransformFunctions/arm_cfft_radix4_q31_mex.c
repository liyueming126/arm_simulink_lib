#include "mex.h"
#include <stdint.h>
#include "arm_cfft_radix4_q31_simulink.c"
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (nrhs < 3) mexErrMsgTxt("Usage: y = arm_cfft_radix4_q31_mex(data, ifftFlag, bitReverseFlag)");
    if (!mxIsInt32(prhs[0]) || mxIsComplex(prhs[0]) || mxIsSparse(prhs[0]))
        mexErrMsgTxt("data must be real int32.");
    ...
    pData = (q31_t *)mxGetData(prhs[0]);
    for (i = 0; i < (mwSize)n; i++) ((q31_t *)mxGetData(plhs[0]))[i] = pData[i];

    arm_cfft_radix4_q31(&S, (q31_t *)mxGetData(plhs[0]));
}