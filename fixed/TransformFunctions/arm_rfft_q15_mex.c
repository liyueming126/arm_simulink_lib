/* ----------------------------------------------------------------------
 * arm_rfft_q15_mex.c
 * MEX wrapper for arm_rfft_q15.
 * Includes arm_cfft_q15_simulink.c and arm_rfft_q15_simulink.c.
 *
 * Usage:
 *   y = arm_rfft_q15_mex(x, ifftFlag)
 *   Forward RFFT (ifftFlag=0):
 *     x: int16 vector of length N (real samples)
 *     y: int16 vector of length 2*N (interleaved complex, N = fftLenReal)
 *   Inverse RIFFT (ifftFlag=1):
 *     x: int16 vector of length 2*N (interleaved complex)
 *     y: int16 vector of length N (real samples)
 * -------------------------------------------------------------------- */
...
/* --------------------------------------------------------------------
 * Generate RFFT twiddle tables A and B (Q15)
 * Based on CMSIS formulas with n_table = 4096 max.
 * -------------------------------------------------------------------- */
static void generate_real_twiddle_q15(q15_t *pA, q15_t *pB, uint32_t nPairs)
{
    uint32_t i;
    ...
    for (i = 0; i < nPairs; i++) {
        ...
        pA[2u * i]     = (q15_t)((int32_t)(a0 * 32768.0 + (a0 >= 0.0 ? 0.5 : -0.5)));
        pA[2u * i + 1u] = (q15_t)((int32_t)(a1 * 32768.0 + (a1 >= 0.0 ? 0.5 : -0.5)));
        pB[2u * i]     = (q15_t)((int32_t)(b0 * 32768.0 + (b0 >= 0.0 ? 0.5 : -0.5)));
        pB[2u * i + 1u] = (q15_t)((int32_t)(b1 * 32768.0 + (b1 >= 0.0 ? 0.5 : -0.5)));
    }
}
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    q15_t *pDataIn, *pDataOut;

    /* Check inputs */
    if (nrhs < 1 || nrhs > 2)
        mexErrMsgIdAndTxt("arm_rfft_q15_mex:invalidInput",
            "Usage: y = arm_rfft_q15_mex(x, ifftFlag)");
    if (nlhs > 1)
        mexErrMsgIdAndTxt("arm_rfft_q15_mex:invalidOutput",
            "At most 1 output allowed.");

    if (!mxIsInt16(prhs[0]) || mxIsComplex(prhs[0]))
        mexErrMsgIdAndTxt("arm_rfft_q15_mex:invalidInputType",
    ...
    if (nrhs >= 2) {
        if (!mxIsDouble(prhs[1]) || mxGetNumberOfElements(prhs[1]) != 1)
            mexErrMsgIdAndTxt("arm_rfft_q15_mex:invalidFlag",
    ...
    if (ifftFlag == 0) {
        ...
        if (dataLen % 2 != 0)
            mexErrMsgIdAndTxt("arm_rfft_q15_mex:invalidDim",
    ...
    if (fftLenReal != 32 && fftLenReal != 64 && fftLenReal != 128 &&
        ...
        mexErrMsgIdAndTxt("arm_rfft_q15_mex:invalidFFTSize",
    ...
    /* Generate A and B twiddle tables (4096 complex pairs = 8192 entries) */
    {
        ...
        pA = (q15_t *)mxCalloc(tblLen * 2u, sizeof(q15_t));
        pB = (q15_t *)mxCalloc(tblLen * 2u, sizeof(q15_t));
        if (!pA || !pB)
            mexErrMsgIdAndTxt("arm_rfft_q15_mex:alloc",
                "Failed to allocate RFFT twiddle tables.");

        generate_real_twiddle_q15(pA, pB, tblLen);
    ...
    /* Set up CFFT instance for the inner complex FFT */
    {
        ...
        S_CFFT = (arm_cfft_instance_q15 *)mxCalloc(1, sizeof(arm_cfft_instance_q15));
        if (!S_CFFT)
            mexErrMsgIdAndTxt("arm_rfft_q15_mex:alloc",
        ...
        {
            q15_t *twiddle = (q15_t *)mxCalloc(twidLen * 2, sizeof(q15_t));
            if (!twiddle)
                mexErrMsgIdAndTxt("arm_rfft_q15_mex:alloc",
            ...
            for (i = 0; i < twidLen; i++) {
                ...
                double c = cos(angle);
                double s = sin(angle);
                twiddle[2u * i]     = (q15_t)((int32_t)(c * 32768.0 + (c >= 0.0 ? 0.5 : -0.5)));
                twiddle[2u * i + 1u] = (q15_t)((int32_t)(s * 32768.0 + (s >= 0.0 ? 0.5 : -0.5)));
            }
    ...
    if (ifftFlag == 0) {
        ...
        plhs[0] = mxCreateNumericMatrix(1, (mwSize)(fftLenReal * 2), mxINT16_CLASS, mxREAL);
        pDataIn = (q15_t *)mxCalloc(fftLenReal, sizeof(q15_t));
        pDataOut = (q15_t *)mxGetData(plhs[0]);

        /* Copy input (real samples) */
        memcpy(pDataIn, pSrc, fftLenReal * sizeof(int16_t));

        /* Run RFFT */
        arm_rfft_q15(&S_RFFT, pDataIn, pDataOut);

        mxFree(pDataIn);
    } else {
        /* Inverse RIFFT: input 2*N complex -> output N real */
        plhs[0] = mxCreateNumericMatrix(1, (mwSize)fftLenReal, mxINT16_CLASS, mxREAL);
        pDataIn = (q15_t *)mxCalloc(fftLenReal * 2, sizeof(q15_t));
        pDataOut = (q15_t *)mxGetData(plhs[0]);

        /* Copy input (complex interleaved spectrum) */
        memcpy(pDataIn, pSrc, fftLenReal * 2 * sizeof(int16_t));
