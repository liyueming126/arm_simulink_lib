function arm_rfft_q15()
clc;
fprintf('=== Testing arm_rfft_q15 RFFT/RIFFT (forward and inverse) ===\n\n');
errors = 0;
...
for idx = 1:length(sizes)
    ...
    x = int16(zeros(1, n));
    for k = 0:n-1
        x(k+1) = int16(round(10000 * sin(2*pi*3*k/n)));  % bin 3 sine
    end

    % Forward RFFT: real -> complex
    y = arm_rfft_q15_mex(x, 0);
    ...
    % RIFFT output is scaled by N internally (the CMSIS code does pDst[i] << 1 at the end)
    % so the output should match the original input (within quantization error)
    diff = max(abs(double(x) - double(z)));
    if diff <= 3
        pass_count = pass_count + 1;
        fprintf('    N=%d: PASS (max diff=%d)\n', n, diff);
    else
        fprintf('    N=%d: FAIL (max diff=%d > 3)\n', n, diff);
        errors = errors + 1;
    end
end
...
x = int16(zeros(1, n));
for k = 0:n-1
    x(k+1) = int16(round(8000 * cos(2*pi*2*k/n) + 5000 * sin(2*pi*5*k/n)));
end
