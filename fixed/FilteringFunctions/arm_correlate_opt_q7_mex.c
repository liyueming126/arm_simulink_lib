...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (nrhs != 2)
    {
        mexErrMsgIdAndTxt("arm_correlate_opt_q7_mex:invalidInput",
            "Two inputs required: signalA (int8), signalB (int8).");
    }

    if (nlhs > 1)
    {
        mexErrMsgIdAndTxt("arm_correlate_opt_q7_mex:invalidOutput",
            "At most one output argument allowed.");
    }

    if (!mxIsInt8(prhs[0]) || mxIsComplex(prhs[0]))
    {
        mexErrMsgIdAndTxt("arm_correlate_opt_q7_mex:invalidInputType",
    ...
    arm_correlate_opt_q7(pSrcA, (uint32_t)srcALen, pSrcB, (uint32_t)srcBLen, pDst, pScratch1, pScratch2);
