...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (nrhs != 2)
    {
        mexErrMsgIdAndTxt("arm_conv_fast_q31_mex:invalidInput",
            "Two inputs required: int32 arrays signalA and signalB.");
    }

    if (nlhs > 1)
    {
        mexErrMsgIdAndTxt("arm_conv_fast_q31_mex:invalidOutput",
            "At most one output argument allowed.");
    }

    if (!mxIsInt32(prhs[0]) || mxIsComplex(prhs[0]))
    {
        mexErrMsgIdAndTxt("arm_conv_fast_q31_mex:invalidInputType",
    ...
    arm_conv_fast_q31(pSrcA, (uint32_t)srcALen, pSrcB, (uint32_t)srcBLen, pDst);
}