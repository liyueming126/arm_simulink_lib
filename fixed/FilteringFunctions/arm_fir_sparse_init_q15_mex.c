...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (nrhs < 3)
        mexErrMsgIdAndTxt("arm_fir_sparse_init_q15_mex:invalidInput","Inputs: numTaps, maxDelay, blockSize");
    ...
    else mexErrMsgIdAndTxt("arm_fir_sparse_init_q15_mex:invalidType","numTaps must be double or int32");
    ...
    else mexErrMsgIdAndTxt("arm_fir_sparse_init_q15_mex:invalidType","maxDelay must be double or int32");
    ...
    else mexErrMsgIdAndTxt("arm_fir_sparse_init_q15_mex:invalidType","blockSize must be double or int32");

    plhs[0] = mxCreateNumericMatrix(1, (mwSize)numTaps, mxINT16_CLASS, mxREAL);
    memset(mxGetData(plhs[0]), 0, numTaps * sizeof(int16_t));
    plhs[1] = mxCreateNumericMatrix(1, (mwSize)(maxDelay + blockSize), mxINT16_CLASS, mxREAL);
    memset(mxGetData(plhs[1]), 0, (maxDelay + blockSize) * sizeof(int16_t));
