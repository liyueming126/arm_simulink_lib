...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (nrhs != 2)
        mexErrMsgIdAndTxt("arm_conv_fast_opt_q15_mex:invalidInput","Inputs: A(int16), B(int16)");

    if (!mxIsInt16(prhs[0]) || mxIsComplex(prhs[0]))
        mexErrMsgIdAndTxt("arm_conv_fast_opt_q15_mex:invalidType","Input 1 must be int16.");
    if (!mxIsInt16(prhs[1]) || mxIsComplex(prhs[1]))
        mexErrMsgIdAndTxt("arm_conv_fast_opt_q15_mex:invalidType","Input 2 must be int16.");
