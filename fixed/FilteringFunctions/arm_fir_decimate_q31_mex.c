...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    S.pCoeffs = (q31_t *)mxMalloc(S.numTaps * sizeof(q31_t));
    for (i = 0; i < S.numTaps; i++) {
        double v = pCoef[i] * 2147483648.0;
        if (v > 2147483647.0) v = 2147483647.0; if (v < -2147483648.0) v = -2147483648.0;
        S.pCoeffs[i] = (q31_t)((int32_t)v);
    }
    blockSize = (uint32_t)(mxGetScalar(prhs[4]));
    S.pState = (q31_t *)mxMalloc((S.numTaps + blockSize - 1) * sizeof(q31_t));
    memset(S.pState, 0, (S.numTaps + blockSize - 1) * sizeof(q31_t));
    ...
    pDst = (q31_t *)mxMalloc(outBlockSize * sizeof(q31_t));
