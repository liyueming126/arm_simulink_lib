...
static arm_status arm_conv_partial_q31(
    q31_t *pSrcA,
    uint32_t srcALen,
    q31_t *pSrcB,
    uint32_t srcBLen,
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (nrhs != 4)
    {
        mexErrMsgIdAndTxt("arm_conv_partial_q31_mex:invalidInput",
            "Four inputs: signalA (int32), signalB (int32), firstIndex, numPoints.");
    }

    if (nlhs > 1)
    {
        mexErrMsgIdAndTxt("arm_conv_partial_q31_mex:invalidOutput",
            "At most one output.");
    }

    if (!mxIsInt32(prhs[0]) || mxIsComplex(prhs[0]))
    {
        mexErrMsgIdAndTxt("arm_conv_partial_q31_mex:invalidInputType",
    ...
    status = arm_conv_partial_q31(pSrcA, (uint32_t)srcALen, pSrcB, (uint32_t)srcBLen,
        pDst, (uint32_t)firstIndex, (uint32_t)numPoints);

    if (status == ARM_MATH_ARGUMENT_ERROR)
    {
        mexErrMsgIdAndTxt("arm_conv_partial_q31_mex:argError",
