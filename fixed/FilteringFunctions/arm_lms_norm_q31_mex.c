...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (nrhs < 3 || nrhs > 5)
        mexErrMsgIdAndTxt("arm_lms_norm_q31_mex:invalidInput",
            "Inputs: x(int32), d(int32), coeffs(int32), [mu(int32)=0x40000000], [postShift(uint8)=0]");

    if (!mxIsInt32(prhs[0]) || mxIsComplex(prhs[0]))
        mexErrMsgIdAndTxt("arm_lms_norm_q31_mex:invalidType","Input 1 (x) must be int32.");
    if (!mxIsInt32(prhs[1]) || mxIsComplex(prhs[1]))
        mexErrMsgIdAndTxt("arm_lms_norm_q31_mex:invalidType","Input 2 (d) must be int32.");
    if (!mxIsInt32(prhs[2]) || mxIsComplex(prhs[2]))
        mexErrMsgIdAndTxt("arm_lms_norm_q31_mex:invalidType","Input 3 (coeffs) must be int32.");
    ...
    if (mxGetNumberOfElements(prhs[1]) != (size_t)blockSize)
        mexErrMsgIdAndTxt("arm_lms_norm_q31_mex:invalidSize","x and d must have same length.");
