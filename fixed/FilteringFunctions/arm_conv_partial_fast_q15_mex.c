...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (nrhs != 4)
    {
        mexErrMsgIdAndTxt("arm_conv_partial_fast_q15_mex:invalidInput",
            "Four inputs: signalA (int16), signalB (int16), firstIndex, numPoints.");
    }

    if (nlhs > 1)
    {
        mexErrMsgIdAndTxt("arm_conv_partial_fast_q15_mex:invalidOutput",
            "At most one output.");
    }

    if (!mxIsInt16(prhs[0]) || mxIsComplex(prhs[0]))
    {
        mexErrMsgIdAndTxt("arm_conv_partial_fast_q15_mex:invalidInputType",
    ...
    status = arm_conv_partial_fast_q15(pSrcA, (uint32_t)srcALen, pSrcB, (uint32_t)srcBLen,
        pDst, (uint32_t)firstIndex, (uint32_t)numPoints);

    if (status == ARM_MATH_ARGUMENT_ERROR)
    {
        mexErrMsgIdAndTxt("arm_conv_partial_fast_q15_mex:argError",
