...
static void arm_correlate_fast_q15(
    q15_t *pSrcA,
    uint32_t srcALen,
    q15_t *pSrcB,
    uint32_t srcBLen,
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (nrhs != 2)
    {
        mexErrMsgIdAndTxt("arm_correlate_fast_q15_mex:invalidInput",
            "Two inputs required: signalA (int16), signalB (int16).");
    }

    if (nlhs > 1)
    {
        mexErrMsgIdAndTxt("arm_correlate_fast_q15_mex:invalidOutput",
            "At most one output argument allowed.");
    }

    if (!mxIsInt16(prhs[0]) || mxIsComplex(prhs[0]))
    {
        mexErrMsgIdAndTxt("arm_correlate_fast_q15_mex:invalidInputType",
    ...
    arm_correlate_fast_q15(pSrcA, (uint32_t)srcALen, pSrcB, (uint32_t)srcBLen, pDst);
}