...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (nrhs != 3)
    {
        mexErrMsgIdAndTxt("arm_fir_fast_q31_mex:invalidInput",
            "Three inputs required: x (int32), coeffs (int32), state (int32 or empty).");
    }

    if (nlhs > 2)
    {
        mexErrMsgIdAndTxt("arm_fir_fast_q31_mex:invalidOutput",
            "At most two outputs: y and updated state.");
    }

    if (!mxIsInt32(prhs[0]) || mxIsComplex(prhs[0]))
    {
        mexErrMsgIdAndTxt("arm_fir_fast_q31_mex:invalidInputType",
    ...
    arm_fir_fast_q31(&S, pSrc, pDst, (uint32_t)blockSize);
