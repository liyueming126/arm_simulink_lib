...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (nrhs != 2)
    {
        mexErrMsgIdAndTxt("arm_conv_q15_mex:invalidInput",
            "Two inputs required: signalA (int16), signalB (int16).");
    }

    if (nlhs > 1)
    {
        mexErrMsgIdAndTxt("arm_conv_q15_mex:invalidOutput",
            "At most one output argument allowed.");
    }

    if (!mxIsInt16(prhs[0]) || mxIsComplex(prhs[0]))
    {
        mexErrMsgIdAndTxt("arm_conv_q15_mex:invalidInputType",
    ...
    arm_conv_q15(pSrcA, (uint32_t)srcALen, pSrcB, (uint32_t)srcBLen, pDst);
}