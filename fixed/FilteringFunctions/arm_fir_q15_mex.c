...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (nrhs != 3)
    {
        mexErrMsgIdAndTxt("arm_fir_q15_mex:invalidInput",
            "Three inputs required: x (int16 array), coeffs (int16 array), state (int16 array or empty).");
    }

    if (nlhs > 2)
    {
        mexErrMsgIdAndTxt("arm_fir_q15_mex:invalidOutput",
            "At most two outputs: y (filter output) and updated state.");
    }

    if (!mxIsInt16(prhs[0]) || mxIsComplex(prhs[0]))
    {
        mexErrMsgIdAndTxt("arm_fir_q15_mex:invalidInputType",
            "First input must be a real int16 array (input signal).");
    }

    if (!mxIsInt16(prhs[1]) || mxIsComplex(prhs[1]))
    {
        mexErrMsgIdAndTxt("arm_fir_q15_mex:invalidInputType",
            "Second input must be a real int16 array (filter coefficients).");
    }
    ...
    pState = (int16_t *)mxGetData(prhs[2]);
    size_t stateLen = mxGetNumberOfElements(prhs[2]);
    ...
    arm_fir_q15(&S, pSrc, pDst, (uint32_t)blockSize);
