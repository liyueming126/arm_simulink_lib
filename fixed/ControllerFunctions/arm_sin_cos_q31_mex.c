...
{
    q31_t theta, sinVal, cosVal;
    int32_t *pOut;

    if (nrhs != 1)
        mexErrMsgIdAndTxt("arm_sin_cos_q31:invalidInput", "1 input required: theta (int32 scalar).");
    if (nlhs > 1)
        mexErrMsgIdAndTxt("arm_sin_cos_q31:invalidOutput", "At most 1 output allowed.");
    if (!mxIsInt32(prhs[0]) || mxIsComplex(prhs[0]) || mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("arm_sin_cos_q31:invalidInputType", "theta must be scalar int32.");

    theta = *(int32_t *)mxGetData(prhs[0]);
    arm_sin_cos_q31(theta, &sinVal, &cosVal);
