...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	if (nrhs != 2)
		mexErrMsgIdAndTxt("arm_mat_sub_q31:invalidInput", "2 inputs required (matrix A, matrix B).");
	if (nlhs > 1)
		mexErrMsgIdAndTxt("arm_mat_sub_q31:invalidOutput", "At most 1 output allowed.");
	if (!mxIsInt32(prhs[0]) || mxIsComplex(prhs[0]) || mxIsSparse(prhs[0]))
		mexErrMsgIdAndTxt("arm_mat_sub_q31:invalidInputType", "Input 1 must be real int32.");
	if (!mxIsInt32(prhs[1]) || mxIsComplex(prhs[1]) || mxIsSparse(prhs[1]))
		mexErrMsgIdAndTxt("arm_mat_sub_q31:invalidInputType", "Input 2 must be real int32.");
	...
	if (rowsA != rowsB || colsA != colsB)
		mexErrMsgIdAndTxt("arm_mat_sub_q31:invalidInput", "Input matrices must have the same dimensions.");
	...
	arm_status status = arm_mat_sub_q31(&srcA, &srcB, &dst);
	if (status == ARM_MATH_SIZE_MISMATCH)
		mexErrMsgIdAndTxt("arm_mat_sub_q31:sizeMismatch", "Matrix dimension mismatch.");
}