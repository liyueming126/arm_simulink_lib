...
#include "arm_cfft_radix4_init_q15_simulink.c"
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs < 3) mexErrMsgTxt("Usage: [S] = arm_cfft_radix4_init_q15_mex(fftLen, ifftFlag, bitReverseFlag)");

    fftLen         = (uint16_t)mxGetScalar(prhs[0]);
    ifftFlag       = (uint8_t)mxGetScalar(prhs[1]);
    bitReverseFlag = (uint8_t)mxGetScalar(prhs[2]);

    arm_cfft_radix4_init_q15(&S, fftLen, ifftFlag, bitReverseFlag);
