...
#include "arm_rfft_init_q15_simulink.c"
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs < 3) mexErrMsgTxt("Usage: [S] = arm_rfft_init_q15_mex(fftLenReal, ifftFlagR, bitReverseFlagR)");

    fftLenReal = (uint16_t)mxGetScalar(prhs[0]);
    ifftFlagR = (uint8_t)mxGetScalar(prhs[1]);
    bitReverseFlagR = (uint8_t)mxGetScalar(prhs[2]);

    arm_rfft_init_q15(&S, fftLenReal, ifftFlagR, bitReverseFlagR);
