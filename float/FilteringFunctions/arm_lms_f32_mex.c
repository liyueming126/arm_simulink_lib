/* MEX wrapper for arm_lms_f32 */
#include "mex.h"
#include <string.h>

#include "arm_lms_f32_simulink.c"
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (nrhs != 4) {
        mexErrMsgIdAndTxt("arm_lms_f32:InvalidInput", "Four inputs: mu, coefficients, input signal, reference signal");
    }
    if (nlhs != 2) {
        mexErrMsgIdAndTxt("arm_lms_f32:InvalidOutput", "Two outputs required: output signal, error signal");
    }
    if (!mxIsSingle(prhs[1]) || !mxIsSingle(prhs[2]) || !mxIsSingle(prhs[3])) {
        mexErrMsgIdAndTxt("arm_lms_f32:NotSingle", "Inputs must be single precision");
    }
