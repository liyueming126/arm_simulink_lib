/* MEX wrapper for arm_lms_init_f32 */
#include "mex.h"
#include <string.h>

#include "arm_lms_init_f32_simulink.c"
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (nrhs != 4) {
        mexErrMsgIdAndTxt("arm_lms_init_f32:InvalidInput", "Four inputs: numTaps, coefficients, mu, blockSize");
    }
    if (nlhs < 1) {
        mexErrMsgIdAndTxt("arm_lms_init_f32:InvalidOutput", "At least one output required");
    }
    if (!mxIsSingle(prhs[1])) {
        mexErrMsgIdAndTxt("arm_lms_init_f32:NotSingle", "Coefficients must be single precision");
    }
